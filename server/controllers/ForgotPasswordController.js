...
// console.log(typeof oldUser)
    ...
    if (oldUser.length === 0) {
      ...
      const token = jwt.sign({ email: oldUser[0].email, id: oldUser[0]._id }, secret, { expiresIn: '15m' });
      const link = `${BASE_URL}/forgot-password/${oldUser[0]._id}/${token}`;
      ...
      let mailOptions = {
        ...
        to: email,
        subject: 'PASSWORD RESET CASINOSTEPS',
...
const handlePostVerified = async (req, res) => {
  ...
  try {
    ...
    const data = await pool.query('SELECT * FROM users WHERE _id = $1', [id])
    const oldUser = data.rows;
    ...
    try {
      ...
      // await User.updateOne({_id: id}, {$set: {password: hashedPwd}})
      await pool.query('UPDATE users SET password=$1 WHERE _id=$2', [hashedPwd, id]) //foundUser[0]._id ????
